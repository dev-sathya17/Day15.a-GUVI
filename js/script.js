...
footer.addEventListener("click", (event) => {
  if (event.target.innerHTML === "Previous") {
    if (page > 2) {
      ...
      footer.children[footer.children.length - 1].removeAttribute("disabled");
    }
    const buttons = footer.querySelectorAll("button");
    buttons.forEach((button) => button.classList.remove("active"));
    buttons[page].classList.add("active");
  } else if (event.target.innerHTML === "Next") {
    footer.children[1].removeAttribute("disabled");
    if (page < 9) {
      page += 1;
      renderData(data, page);
    } else if (page === 9) {
    ...
    page = 1;
    renderData(data, page);
    footer.children[1].setAttribute("disabled", true);
    footer.children[footer.children.length - 1].removeAttribute("disabled");
  } else {
    const buttons = footer.querySelectorAll("button");
    buttons.forEach((button) => button.classList.remove("active"));
    ...
    buttons[page].classList.add("active");
    renderData(data, page);
    if (parseInt(event.target.innerHTML) === 10) {
      footer.children[footer.children.length - 1].setAttribute(
        "disabled",
        true
      );
      footer.children[1].removeAttribute("disabled");
...
function renderData(data, page) {
  ...
  tbody.innerHTML = "";
  for (let index = 10 * (page - 1); index < 10 * page; index++) {
...
function createFooter(footer, data) {
  ...
  for (let i = 2; i <= Math.ceil(data.length / 10); i++) {
    const child = {};
  ...
  create(footerElements);
}

function create(params) {
  for (let param of params) {
    const tag = document.createElement(param.tagName);
    tag.innerHTML = param.name;
    if (param.attribute) {
      tag.setAttribute(param.attribute[0], param.attribute[1]);
    }
    if (param.className) {
      tag.classList.add(param.className);
    }
    param.parent.appendChild(tag);
  }
}
